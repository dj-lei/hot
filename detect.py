...
if __name__ == "__main__":
    ...
    all_images_csv = pd.read_csv(opt.image_csv)
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                train_images_values.append(all_images_csv.loc[(all_images_csv['id'] == filename), 'value'].values[0])

    train_data = pd.DataFrame(pd.Series(train_images_path), columns=['path'])
    train_data['value'] = pd.Series(train_images_values)
